...
Dieses Skript baut eine vollständige Haystack-v2 Pipeline basierend auf einer JSON-Definitionsdatei
(z.B. awr_diff_pipeline.json oder sample_pipeline.json) auf, generiert die Python-Komponentencodes
sowie das Ablaufskript und führt die Pipeline direkt aus.

Verwendung:
  python3 run_pipeline.py [PFAD_ZUR_JSON_PIPELINE]

Beispiel:
  python3 run_pipeline.py awr_diff_pipeline.json
"""
...
# ==============================================================================
# 1. Metadaten & Python-Klassen-Templates für Custom-Komponenten
# ==============================================================================
...
# Python-Implementationen für alle Custom-Komponenten (gespiegelt aus dem Builder)
CUSTOM_CLASSES = {
    "WebServerLogLoader": """
class WebServerLogLoader(Component):
    \"\"\"Lädt Webserver-Zugriffslogs (combined/common/json) und erzeugt Documents.\"\"\"
    def __init__(self, log_path="/var/log/nginx/access.log", format="combined", max_lines=100000, **kwargs):
        super().__init__()
...
class OracleAWRDiffParser(Component):
    ...
    def run(self, documents: List[Document] = None):
        ...
        for d in documents or []:
            ...
            if db_time_m:
                load_profile.append({
                    "metric": "DB Time (s)", "period_1": db_time_m.group(1),
                    "period_2": db_time_m.group(2), "diff_pct": db_time_m.group(3)
                })
                
            hard_parses_m = re.search(r"Hard parses.*?([\\d\\.]+).*?([\\d\\.]+).*?([\\d\\.]+)", content, re.IGNORECASE | re.DOTALL)
            if hard_parses_m:
                load_profile.append({
                    "metric": "Hard parses", "period_1": hard_parses_m.group(1),
                    "period_2": hard_parses_m.group(2), "diff_pct": hard_parses_m.group(3)
                })
                
            phys_reads_m = re.search(r"Physical reads.*?([\\d\\.]+).*?([\\d\\.]+).*?([\\d\\.]+)", content, re.IGNORECASE | re.DOTALL)
            if phys_reads_m:
                load_profile.append({
                    "metric": "Physical reads", "period_1": phys_reads_m.group(1),
                    "period_2": phys_reads_m.group(2), "diff_pct": phys_reads_m.group(3)
                })

            events_rx = re.findall(r"<tr>\\s*<td>([^<]+)</td>\\s*<td>([^<]+)</td>\\s*<td>([\\d\\.]+)</td>\\s*<td>([^<]+)</td>\\s*</tr>", content)
            for ev in events_rx[:self.max_wait_events]:
                wait_events.append({
                    "event": ev[0].strip(), "class": ev[1].strip(),
                    "waits_diff_pct": ev[2].strip(), "time_diff_info": ev[3].strip()
                })

            sql_rx = re.findall(r"SQL Id:\\s*(\\w+)\\s*-\\s*([^<]+?)\\s*\\(Elapsed Time:\\s*([^,]+),\\s*([^)]+)\\)", content, re.IGNORECASE)
            for sq in sql_rx[:self.max_sql_queries]:
                sql_queries.append({
                    "sql_id": sq[0].strip(), "statement": sq[1].strip(),
                    "elapsed_time": sq[2].strip(), "pct_db_time": sq[3].strip()
                })
    ...
    def run(self, documents: List[Document] = None):
        ...
        for d in documents or []:
            ...
            try:
                ...
                # Common/Combined parsing
                m = re.match(r'(\\S+) \\S+ \\S+ \\[([^\\]]+)\\] "(\\S+) (\\S+) (\\S+)" (\\d+) (\\d+|-)', content)
                if m:
                    meta.update({
...
class TimeWindowFilter(Component):
    \"\"\"Behält nur Log-Zeilen innerhalb eines definierten Testzeitraums.\"\"\"
    def __init__(self, start, end, timestamp_field="timestamp", **kwargs):
        super().__init__()
...
class KeywordFilter(Component):
    \"\"\"Durchsucht Dokumente nach Schlüsselwörtern.\"\"\"
...
class MetricAggregator(Component):
    \"\"\"Aggregiert Zeitreihen-Metriken (Mittelwert, P95, Max).\"\"\"
...
class ErrorClassifier(Component):
    \"\"\"Fasst gefundene Fehler in Fehlerklassen zusammen.\"\"\"
    ...
    def run(self, matches: List[Document] = None, documents: List[Document] = None):
        ...
        summary = {"classes": dict(counter), "ok_label": self.ok_label, "total": sum(counter.values())}
...
class TimeframeErrorCorrelator(Component):
    ...
    def __init__(self, error_pattern="fatal|error|500|ora-", window_seconds=120, timestamp_field="timestamp", **kwargs):
        super().__init__()
    ...
    def run(self, primary_logs: List[Document] = None, secondary_logs: List[Document] = None):
        ...
        anchors = []
        for d in primary:
            if rx.search(d.content or ""):
                dt = parse_ts(d.meta.get(self.timestamp_field))
                if dt: anchors.append((d, dt))
        ...
        for anchor, anchor_dt in anchors:
            if anchor.id not in seen_ids:
                ...
                meta.update({"is_correlation_anchor": True, "correlation_role": "anchor"})
                correlated.append(Document(id=anchor.id, content=anchor.content, meta=meta))
                seen_ids.add(anchor.id)

            for sec_doc in secondary:
                sec_dt = parse_ts(sec_doc.meta.get(self.timestamp_field))
                if sec_dt and abs((sec_dt - anchor_dt).total_seconds()) <= self.window_seconds:
                    meta = dict(sec_doc.meta)
                    meta.update({"is_correlated": True, "correlation_role": "secondary", "correlation_anchor_id": anchor.id})
                    doc_id = f"{sec_doc.id}_corr_{anchor.id}"
                    if doc_id not in seen_ids:
                        correlated.append(Document(id=doc_id, content=sec_doc.content, meta=meta))
                        seen_ids.add(doc_id)

        return {"correlated_documents": correlated, "summary": {"anchors_found": len(anchors), "total_correlated": len(correlated)}}
"""
}

# ==============================================================================
# 2. Generierungs- & Übersetzungsfunktionen
# ==============================================================================
...
def generate_python_files(pipeline_data):
    """Generiert pipeline_components_run.py und pipeline_pipeline_run.py auf der Festplatte."""
    nodes = pipeline_data.get("nodes", [])
    used_types = set(n["type"] for n in nodes)
    ...
    for t in sorted(used_types):
        if t in HAYSTACK_BUILTIN:
            continue
        src = CUSTOM_CLASSES.get(t)
        if src:
            comp_code.append(src)
        else:
            # Fallback
            comp_code.append(f"""
class {t}(Component):
...
""")
    ...
    with open("pipeline_components_run.py", "w", encoding="utf-8") as f:
        f.write("\n".join(comp_code))
    print("✓ 'pipeline_components_run.py' wurde erfolgreich generiert.")

    # Generate Pipeline Graph Script
    ...
    for n in nodes:
        ...
        if t == "OpenAIGenerator":
            if "api_base_url" in settings:
                params["api_base_url"] = params.get("api_base_url") or settings["api_base_url"]
            if "api_key" in settings:
                params["api_key"] = params.get("api_key") or settings["api_key"]
                
        cls_name = t
        if t in HAYSTACK_BUILTIN:
            imports.add(HAYSTACK_IMPORTS[t])
            cls_name = HAYSTACK_BUILTIN[t]
            
        pstr = ", ".join(f"{k}={py_value(v)}" for k, v in params.items())
        comp_inits.append(f'    builder.add_component("{n["id"]}", {cls_name}({pstr}))')
        
    conns = pipeline_data.get("connections", [])
    edge_lines = []
    for c in conns:
        edge_lines.append(f'    builder.connect("{c["from"]}.{c["from_port"]}", "{c["to"]}.{c["to_port"]}")')
        
    # Find entry
    entry_id = "awr_loader" if "awr_loader" in [n["id"] for n in nodes] else (nodes[0]["id"] if nodes else "ingest")
    
    imports_block = "\n".join(sorted(imports)) if imports else "# Keine eingebauten Komponenten"
    
    pipeline_code = f"""\"\"\"
Automatisch generierter Pipeline-Graph.
\"\"\"
import json
from haystack import Pipeline
{imports_block}
from pipeline_components_run import *

def build_pipeline():
    builder = Pipeline()
{chr(10).join(comp_inits)}
{chr(10).join(edge_lines)}
    return builder

if __name__ == "__main__":
    p = build_pipeline()
    print("✓ Pipeline erfolgreich gebaut!")
    # Ausführen
    entry = "{entry_id}"
    print(f"Ausführung der Pipeline gestartet (Einstiegspunkt: {{entry}})...")
    try:
        res = p.run({{entry: {{}}}})
        print("\\n=== PIPELINE-ERGEBNIS ===")
        print(json.dumps(res, indent=2, ensure_ascii=False, default=str)[:3000])
    except Exception as e:
        print(f"Fehler bei Pipeline-Ausführung: {{e}}")
        print("Hinweis: Stellen Sie sicher, dass 'haystack-ai' installiert ist und API-Keys gesetzt sind.")
"""

    with open("pipeline_pipeline_run.py", "w", encoding="utf-8") as f:
        f.write(pipeline_code)
    print("✓ 'pipeline_pipeline_run.py' wurde erfolgreich generiert.")
...
def main():
    json_path = "awr_diff_pipeline.json"
    if len(sys.argv) > 1:
        json_path = sys.argv[1]
        
    if not os.path.exists(json_path):
        print(f"❌ Fehler: Datei '{json_path}' wurde nicht gefunden.")
        print("Bitte geben Sie eine gültige JSON-Datei an.")
        sys.exit(1)
        
    print(f"Lese Pipeline-Definition aus '{json_path}'...")
    try:
    ...
    # Python-Klassen und Pipeline-Skripte erzeugen
    generate_python_files(data)
    
    print("\nAusführungsbereites System steht bereit!")
    print("Um die Pipeline zu starten, führen Sie folgendes Kommando aus:")
    print(f"  👉 python3 pipeline_pipeline_run.py\n")
